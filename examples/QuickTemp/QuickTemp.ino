/*
 * Temperature Sensing using NotecardPseudoSensor
 *
...
void loop() {
  // Sample NotecardPseudoSensor temperature
  float temperature = sensor.temp();

  // Log results to Arduino IDE Serial Monitor (Ctrl+Shift+M)
  notecard.logDebugf("INFO: Notecard Temperature: %f.\n", temperature);

  // Notecard only captures a new temperature sample every five minutes,
  // but we can only wait 65s between loops (65535 max on Uno).
  ::delay(65535);
}
